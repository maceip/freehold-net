...
public:
	...
	IntFp_JQv1(IntFp_JQv1 *obj) {
		this->value = obj->value;
	}
	
	IntFp_JQv1(uint64_t input, int party = PUBLIC) {
		if (party == PUBLIC) {
			value = ZKFpExec::zk_exec->pub_label(input);
		} else {
			ZKFpExec::zk_exec->feed_JQv1(value, input);
		}
	}
	static void finalize_inner_product(int w_length, int batch_sz) {
		ZKFpExec::zk_exec->finalize_inner_product_JQv1(w_length, batch_sz);
	}

	IntFp_JQv1 operator*(const IntFp_JQv1& rhs) const {
		IntFp_JQv1 res(*this);
		res.value = ZKFpExec::zk_exec->mul_gate_JQv1(this->value, rhs.value);
		return res;
	}

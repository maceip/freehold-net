...
public:
	...
	uint64_t *diff = nullptr, *diff_tmp = nullptr;
	...
	void matrix_mul_pre_processing_setup_for_JQv1(int matrix_sz, int batch_sz) {
		...
		val_pre_pro = new uint64_t[mul_sz_batch_sz + batch_sz];
		diff = new uint64_t[2 * test_n_batch_sz + 3 * mul_sz_batch_sz + w_length_batch_sz];

		//batch multiplication
		for (int tm = 0; tm < batch_sz; tm++) {
			//extend test_n * 2 for committing mat_a and mat_b's inputs
			//extend 3 * mul_sz_batch_sz for multiplcation triples
			// le_in ri_in out
			int loc = 2 * test_n + tm * (2 * test_n + 3 * mul_sz);
			//val_pre_pro for mult gates
			for (int i = tm * mul_sz; i < (tm + 1) * mul_sz; i++, loc++) {
				val_pre_pro[i] = mult_mod(LOW64(vole_val[loc]), LOW64(vole_val[loc + mul_sz]));
				__uint128_t val = (party == ALICE ? auth_compute_mul_send(vole_val[loc], vole_val[loc + mul_sz]) : 
													auth_compute_mul_recv(vole_val[loc], vole_val[loc + mul_sz]));
				val_pre_pro[i] = add_mod(val_pre_pro[i], LOW64(val));
				val_pre_pro[i] = PR - val_pre_pro[i];
			}
		}
		if (check_cnt) {
			andgate_correctness_check_manage();
		...
		//batch inner product
		for (int i = 0, loc = 2 * test_n_batch_sz + 3 * mul_sz_batch_sz; i < batch_sz; i++, loc += w_length) {
			//extend w_length_batch_sz for committing left randomness
			//val_pre_pro for inner products 0
			auth_compute_inner(vole_val + i * (2 * test_n + 3 * mul_sz), vole_val + loc, w_length - 1, val_pre_pro[mul_sz_batch_sz + i]);
			val_pre_pro[mul_sz_batch_sz + i] = add_mod(val_pre_pro[mul_sz_batch_sz + i], PR - LOW64(vole_val[loc + w_length - 1]));
		}
		if (check_cnt) {
	...
	uint64_t auth_compute_inner(__uint128_t *Ma, __uint128_t *Mb, int len, uint64_t &A0) {
		...
		if (party == ALICE) {
			uint64_t d = 0, A1 = 0;
			for (int i = 0; i < len; i++)
				d = add_mod(d, mult_mod(HIGH64(Ma[i]), HIGH64(Mb[i])));
			d = PR - d;
			d = add_mod(HIGH64(mac), d);
			io->send_data(&d, sizeof(uint64_t));
			for (int i = 0; i < len; i++) {
				A0 = add_mod(A0, mult_mod(LOW64(Ma[i]), LOW64(Mb[i])));
				A1 = add_mod(A1, mult_mod(HIGH64(Ma[i]), LOW64(Mb[i])));
				A1 = add_mod(A1, mult_mod(LOW64(Ma[i]), HIGH64(Mb[i])));
			}
			A0_buffer[check_cnt] = A0;
			A1_buffer[check_cnt] = add_mod(A1, PR - LOW64(mac));
		} else {
			uint64_t d;
			io->recv_data(&d, sizeof(uint64_t));
			for (int i = 0; i < len; i++)
				A0 = add_mod(A0, mult_mod(LOW64(Ma[i]), LOW64(Mb[i])));
			mac = add_mod(mac, mult_mod(d, delta));
			A0_buffer[check_cnt] = add_mod(A0, mult_mod(LOW64(mac), delta));
		}
		A0 = PR - add_mod(LOW64(mac), A0);
		check_cnt++;
		return LOW64(mac);
	}
	...
	__uint128_t authenticated_val_input_with_setup(uint64_t w) {
		__uint128_t mac = *(vole_tmp++);
		*diff_tmp = add_mod(HIGH64(mac), PR - w);
		io->send_data(diff_tmp++, sizeof(uint64_t));
		return (__uint128_t)makeBlock(w, LOW64(mac));
	}

	__uint128_t authenticated_val_input_with_setup() {
		__uint128_t key = *(vole_tmp++);
		io->recv_data(diff_tmp, sizeof(uint64_t));
		key = add_mod(key, mult_mod(*(diff_tmp++), delta));
		return key;
	}
	...
	__uint128_t auth_compute_mul_send_with_setup(__uint128_t Ma, __uint128_t Mb) {
		if(check_cnt == BUFFER_SZ/CHECK_DIV_SZ) {
			andgate_correctness_check_manage_JQv1();
			check_cnt = 0;
		}

		__uint128_t mac = *(vole_tmp++);
		uint64_t A0 = *(val_tmp++);
		uint64_t Ma_da = add_mod(LOW64(Ma), *(diff_tmp - double_mul_sz)), Mb_db = add_mod(LOW64(Mb), *(diff_tmp - mul_sz));
		A0 = add_mod(A0, mult_mod(Ma_da, Mb_db));
		
		uint64_t d = mult_mod(HIGH64(Ma), HIGH64(Mb));
		*diff_tmp = add_mod(HIGH64(mac), PR - d);
		io->send_data(diff_tmp++, sizeof(uint64_t));

		mac = (__uint128_t)makeBlock(d, LOW64(mac));
		A0_buffer[check_cnt++] = add_mod(LOW64(mac), A0);
		return mac;
	}

	__uint128_t auth_compute_mul_recv_with_setup() {
		if(check_cnt == BUFFER_SZ/CHECK_DIV_SZ) {
			andgate_correctness_check_manage_JQv1();
			check_cnt = 0;
		}
		...
		uint64_t A0 = *(val_tmp++);
		uint64_t Ka_da = add_mod(*(vole_tmp - double_mul_sz), *(diff_tmp - double_mul_sz));
		uint64_t Kb_db = add_mod(*(vole_tmp - mul_sz), *(diff_tmp - mul_sz));
		uint64_t da_db = mult_mod(*(diff_tmp - double_mul_sz), *(diff_tmp - mul_sz));
		A0 = add_mod(A0, mult_mod(Ka_da, Kb_db));
		A0 = add_mod(A0, mult_mod(da_db, delta));

		io->recv_data(diff_tmp, sizeof(uint64_t));
		__uint128_t key = *(vole_tmp++);
		key = add_mod(key, mult_mod(*(diff_tmp++), delta));

		A0_buffer[check_cnt++] = add_mod(key, A0);
		return key;
	}

	void finalize_inner_product_JQv1(int w_length, int batch_sz) {
		//At this time, all the val_tmp for mult gates have been comsumed.
		io->flush();
		int w_length_minus_one = w_length - 1;
		// block hash_output, output_recv;
		uint64_t *hash_input = new uint64_t[batch_sz];
		__uint128_t *p_va = vole_val, *p_vb = vole_val + batch_sz * (w_length - 1);
		uint64_t *p_da = diff, *p_db = diff + batch_sz * (w_length - 1);
		if (party == ALICE) {
			for (int tm = 0; tm < batch_sz; tm++) {
				uint64_t A0 = *(val_tmp++), va_da, vb_db;
				for (int i = 0; i < w_length_minus_one; i++, p_va++, p_vb++, p_da++, p_db++) {
					va_da = add_mod(LOW64(*p_va), *p_da);
					vb_db = add_mod(LOW64(*p_vb), *p_db);
					A0 = add_mod(A0, mult_mod(va_da, vb_db));
				}
				p_vb++;
				p_db++;
				hash_input[tm] = A0;
			...
			// hash_output = Hash::hash_for_block(hash_input, batch_sz * sizeof(uint64_t));
			blake3_hasher_update(&hasher, hash_input, batch_sz * sizeof(uint64_t));
        	blake3_hasher_finalize(&hasher, output, BLAKE3_OUT_LEN);
			io->send_data(&output, BLAKE3_OUT_LEN);
		} else {
			for (int tm = 0; tm < batch_sz; tm++) {
				uint64_t A0 = *(val_tmp++), va_da, vb_db, sigma_d_mul = 0;
				for (int i = 0; i < w_length_minus_one; i++, p_va++, p_vb++, p_da++, p_db++) {
					va_da = add_mod(LOW64(*p_va), *p_da);
					vb_db = add_mod(LOW64(*p_vb), *p_db);
					A0 = add_mod(A0, mult_mod(va_da, vb_db));
					sigma_d_mul = add_mod(sigma_d_mul, mult_mod(*p_da, *p_db));
				}
				p_vb++;
				sigma_d_mul = add_mod(sigma_d_mul, PR - *(p_db++));
				hash_input[tm] = add_mod(A0, mult_mod(sigma_d_mul, delta));
			}
		...
		if (party == BOB && memcmp(output, output_recv, BLAKE3_OUT_LEN) != 0) {
			error("inner products check fails");
			std::cout<<"Wrong on JQv1 inner products checks!\n";
		}
	}
	...
	void andgate_correctness_check_manage_JQv1() {
		io->flush();
		
		// block hash_output = Hash::hash_for_block(A0_buffer, check_cnt * sizeof(uint64_t)), output_recv;
		blake3_hasher_update(&hasher, A0_buffer, check_cnt * sizeof(uint64_t));
        blake3_hasher_finalize(&hasher, output, BLAKE3_OUT_LEN);
		if(party == ALICE) 
			io->send_data(&output, BLAKE3_OUT_LEN);
		else
		...
		if (party == BOB && memcmp(output, output_recv, BLAKE3_OUT_LEN) != 0) {
			std::cout<<"Wrong on JQv1 multiplication gates checks!\n";
			error("multiplication gates check fails");
		}
	}

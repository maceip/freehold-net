...
public:
	...
	IntFp_JQv2(IntFp_JQv2 *obj) {
		this->value = obj->value;
	}
	
	IntFp_JQv2(uint64_t input, int party = PUBLIC) {
		if (party == PUBLIC) {
			value = ZKFpExec::zk_exec->pub_label(input);
		} else {
			ZKFpExec::zk_exec->feed_JQv2(value, input, diff);
		}
	}
	static void finalize_inner_product(int w_length, int batch_sz) {
		ZKFpExec::zk_exec->finalize_inner_product_JQv2(w_length, batch_sz);
	}

	IntFp_JQv2 operator*(const IntFp_JQv2& rhs) const {
		IntFp_JQv2 res(*this);
		res.value = ZKFpExec::zk_exec->mul_gate_JQv2(this->value, rhs.value, this->diff, rhs.diff);
		return res;
	}

	IntFp operator*(const uint64_t& rhs) const {
		IntFp res(this->value);
		res.value = ZKFpExec::zk_exec->mul_const_gate(this->value, rhs);
		return res;
	}
};

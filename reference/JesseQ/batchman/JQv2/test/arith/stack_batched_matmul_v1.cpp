...
void test_circuit_zk(BoolIO<NetIO> *ios[threads], int party, int matrix_sz, int branch_sz, int batch_sz) {
    ...
    ZKFpExec::zk_exec->setup_JQv2(matrix_sz, batch_sz);
    auto start = clock_start();

	IntFp_JQv2 *mat_a = new IntFp_JQv2[test_n_batch_sz];
	IntFp_JQv2 *mat_b = new IntFp_JQv2[test_n_batch_sz];
    IntFp_JQv2 *mul_le = new IntFp_JQv2[mul_sz_batch_sz];
	IntFp_JQv2 *mul_ri = new IntFp_JQv2[mul_sz_batch_sz];
	IntFp_JQv2 *mul_ou = new IntFp_JQv2[mul_sz_batch_sz];
    for (int tm = 0; tm < batch_sz; tm++) {
        // Input
        int tm_end = (tm + 1) * test_n;
        for (int i = tm * test_n; i < tm_end; i++)
            mat_a[i] = IntFp_JQv2(1, ALICE);
        for (int i = tm * test_n; i < tm_end; i++)
            mat_b[i] = IntFp_JQv2(1, ALICE);

        ZKFpExec::zk_exec->reverse_flag();
        // Left, Right, Output of MUL gates
        tm_end = (tm + 1) * mul_sz;
        for (int i = tm * mul_sz; i < tm_end; i++)
            mul_le[i] = IntFp_JQv2(1, ALICE);
        for (int i = tm * mul_sz; i < tm_end; i++)
            mul_ri[i] = IntFp_JQv2(1, ALICE);
        for (int i = tm * mul_sz; i < tm_end; i++)
    ...
    IntFp_JQv2 *left_v = new IntFp_JQv2[w_length_batch_sz];
    for (int bid = 0; bid < batch_sz; bid++) 
        for (int i = 0; i < w_length; i++) 
            left_v[bid*w_length + i] = IntFp_JQv2(left_vec_a[i], ALICE);
	...
	IntFp_JQv2::finalize_inner_product(w_length, batch_sz);
